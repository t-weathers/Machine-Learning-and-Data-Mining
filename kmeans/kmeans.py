...
def imageProblem():
  ...
  print("SSE when k = " + str(k) + ": " + str(SSE[-1]))
  # Visualize Clusters
...
def initalizeCentroids(dataset, k):
  ...
  for i in range(0,k):
    centroids[i] = dataset[random.randint(0,dataset.shape[0])]
  return centroids
...
def computeAssignments(dataset, centroids):
  ...
  for i in range(0,dataset.shape[0]):
    #get distances from norm
    distances = np.linalg.norm(dataset[i] - centroids,axis=1)
    #add another dimension to y
    distances = distances[:,np.newaxis]
    #find closest distance (centroid)
    min = np.argmin(distances)
    #set assignment to nearest centroid
    assignments[i] = min
  #print("shape: " + str(assignments.shape))
  return assignments
...
def updateCentroids(dataset, centroids, assignments):
  #print("shape of d: " + str(dataset.shape[1]))
  #print("shape of k: " + str(centroids.shape[0]))
  ...
  for i in range(0,len(centroids)):
    ...
    for k in range(0,dataset.shape[0]):
      #if the centroid matches the assignment
      if i == assignments[k]:
          #increment sums
          xsums += dataset[k][0]
          ysums += dataset[k][1]
    ...
    #divide to get mean
    centroids[i][0] = xsums/counts[i]
    centroids[i][1] = ysums/counts[i]

  #print("Shape Centroids: " + str(centroids.shape))
  #print("Shape Counts: " + str(counts.shape))
...
def calculateSSE(dataset, centroids, assignments):
  ...
  sse = 0.0
  for i in range(0,dataset.shape[0]):
    #find index of centroids
    index = int(assignments[i][0])
    #increment SSE
    sse += np.linalg.norm(dataset[i] - centroids[index])
  return sse
